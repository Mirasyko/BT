...
def optimalizace(supply_it, demand_jt):
    ...
    w_ij = np.array([[m.addVar(vtype=gu.GRB.CONTINUOUS, lb=0) for j in jrange] for i in irange])
    ...
    # alokacni klic je nyni promenna:
    for i in irange:
        m.addConstr(gu.quicksum(w_ij[i, j] for j in jrange) == 1.)
    ...
    print("ALOKACNI KLIC:")
    for i in irange:
        print(i, ": ", [w_ij[i, j].x for j in jrange], np.sum([w_ij[i, j].x for j in jrange]))
    print("")
